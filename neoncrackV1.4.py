#!/usr/bin/env python3
"""
NeonCrack v1.3 - Cyberpunk WPA Handshake Cracking Suite
Author: Niko DeRuise

USAGE:
sudo python3 neoncrackV1.3.py
...
# === Initialize Window ===
root = Tk()
root.title("🧬 NeonCrack v1.3 - Cyberpunk Cracking Suite")
...
def capture_gui():
    ...
    Button(frame, text="📡 Start Capture", command=start_capture, bg=accent, fg="white", font=font_main).pack(pady=10)
...
def start_capture():
    ...
    output_file = f"neoncrack_captures/capture_{timestamp}.pcap"
    capture_output.insert(END, f"[*] Capturing handshakes to {output_file}...\n")

    cmd = ["airodump-ng", "-w", output_file.replace(".pcap", ""), "--output-format", "pcap", iface]
