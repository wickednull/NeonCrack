#!/usr/bin/env python3
"""
NeonCrack v2.4-r3 – live logs, picker fix & Wifite-style WPS flag
...
root = Tk(); root.title("NeonCrack v2.4-r3"); root.configure(bg=BGC); root.geometry("1080x780")
...
def run(cmd, outfile=None):
    ...
    return subprocess.Popen(cmd, stdout=f, stderr=subprocess.STDOUT, preexec_fn=os.setsid)
...
def detect_wps(mon_iface, channel_list, dwell=3):
    ...
    Returns a set of BSSIDs that advertise WPS.
    """
    hits=set()
