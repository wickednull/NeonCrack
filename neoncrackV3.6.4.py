#!/usr/bin/env python3
"""
NeonCrack v3.6.4  –  Wi-Fi Tactical ToolKit!
created by NeonCrack
─────────────────────────────────────────────────────
...
root = Tk(); root.title("NeonCrack v3.6.4"); root.configure(bg=BGC); root.geometry("1080x850")
...
sticky_mon           = BooleanVar(value=False)      # Sticky-Monitor lives in Utilities
...
def start_wifiphisher():
    ...
    global attack_proc
    attack_proc = run_logged(cmd, att_out)
...
for txt,fn in [
    ...
    ("Wifiphisher Evil-Portal", start_wifiphisher),     # ← new button
