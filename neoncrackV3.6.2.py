#!/usr/bin/env python3
"""
NeonCrack v3.6.2 – Wi-Fi Tactical Toolkit
...
WHITELIST = set()                # add iface names to filter dropdown if desired

root = Tk()
root.title("NeonCrack v3.6.2")
...
sticky_mon           = BooleanVar(value=False)        # ← NEW
...
def set_monitor(iface, enable=True):
    """Enable/disable monitor mode and sync iface_var"""
    global monitor_flag
    if not iface: return
...
def restore_monitor():
    """Turn off monitor only if Sticky-Monitor box is unticked."""
...
def stop_bw_monitor():
    bw_stop.set(); log(stats_out,"[*] Monitor stopped")
...
# ── attack routines (PMKID, Handshake, WPS, Deauth, Beacon, Sweep, KARMA) ──
def start_pmkid():
...
# ── cracking / utilities ────────────────────────────────────────────────────
def browse_pcap(): pcap_var.set(
    filedialog.askopenfilename(filetypes=[("Capture","*.cap *.pcap *.pcapng *.hccapx")]))

def browse_word(): word_var.set(
    filedialog.askopenfilename(filetypes=[("Wordlist","*.txt *.lst")]))
...
for k,lbl in [("scan","⚡ Scan"),("attack","⚔️ Attacks"),
    ...
    f=Frame(nb,bg=BGC); nb.add(f,text=lbl); tabs[k]=f
# Scan tab
ts=tabs["scan"]
...
Checkbutton(row,text="Sticky Monitor",variable=sticky_mon,bg=BGC,fg=NEON,
            selectcolor=BGC,activebackground=BGC).grid(row=0,column=8,padx=6)
Frame(ts,height=2,bg=NEON).pack(fill=X,pady=6)
# nmap panel
nmap=Frame(ts,bg=BGC); nmap.pack(fill=X,pady=2)
Label(nmap,text="nmap Target",bg=BGC,fg=NEON).grid(row=0,column=0,padx=4)
Entry(nmap,textvariable=nmap_target,width=18).grid(row=0,column=1)
OptionMenu(nmap,nmap_profile,"Quick Ping","Top-100 Ports","Full TCP",
           "OS Detect","Vuln Script","Custom").grid(row=0,column=2,padx=4)
custom=Entry(nmap,textvariable=nmap_custom,width=22,state="disabled")
custom.grid(row=0,column=3,padx=4)
def toggle(*_): custom.config(state="normal" if nmap_profile.get()=="Custom" else "disabled")
nmap_profile.trace_add("write",toggle)
Button(nmap,text="Run nmap",bg=ACCENT,fg="white",
...
stats_out=scrolledtext.ScrolledText(st,width=115,height=12,bg="#0d0d17",
                                    fg=NEON,font=("Consolas",10))
stats_out.pack()
bw=Frame(st,bg=BGC); bw.pack(pady=10,fill=X)
Label(bw,text="Broadband iface:",bg=BGC,fg=NEON).pack(side=LEFT)
Entry(bw,textvariable=mon_iface_var,width=12).pack(side=LEFT,padx=4)
Button(bw,text="Start",bg=ACCENT,fg="white",
       command=start_bw_monitor).pack(side=LEFT,padx=4)
Button(bw,text="Stop",bg="#ff0030",fg="white",
