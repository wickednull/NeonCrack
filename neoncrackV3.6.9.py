#!/usr/bin/env python3
"""
NeonCrack v3.6.9 – WiFi Tactical ToolKit
...
FONT      = ("Courier New", 11)
FONT_BTN  = ("Courier New", 8)            # compact button font
CAP_DIR   = "neoncrack_captures"; os.makedirs(CAP_DIR, exist_ok=True)

root = Tk(); root.title("NeonCrack v3.6.9"); root.configure(bg=BGC); root.geometry("1080x850")
...
lf_rogue = ttk.LabelFrame(scroll_f,text="🪪 Rogue AP / Phish",style="Nc.TLabelframe"); lf_rogue.pack(fill=X,padx=8,pady=4)
lf_dist  = ttk.LabelFrame(scroll_f,text="⚔️ Disruption",      style="Nc.TLabelframe"); lf_dist.pack(fill=X,padx=8,pady=4)
...
Button(scroll_f,text="Stop Attack",bg="#ff0030",fg="white",
       font=FONT_BTN,command=stop_attack).pack(fill=X,padx=20,pady=6)
...
Button(inp_row,text="Send",bg=ACCENT,fg="white",
       font=FONT_BTN,command=send_to_proc).pack(side=LEFT,padx=6)
