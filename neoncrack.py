#!/usr/bin/env python3
"""
NeonCrack – created by Null_lyfe
──────────────────────────────────────
...
WHITELIST = {"wlan0", "wlan1"}     # leave empty to show all ifaces

root = Tk(); root.title("NeonCrack v3.5"); root.configure(bg=BGC); root.geometry("1080x850")
...
nmap_target, nmap_profile = StringVar(), StringVar(value="Quick Ping")
...
def iw_interfaces():
    try:
        ...
        all_if = [l.split()[1] for l in o.splitlines() if l.strip().startswith("Interface")]
...
def detect_wps(mon_iface, channels):
    hits=set()
...
def do_scan(channel_hop=False):
    ...
    p=run_logged(cmd,scan_out); log(scan_out,"[*] scanning…")
    ...
    log(scan_out,f"[+] {len(networks)} networks."); set_monitor(mon,False)
...
def start_nmap_scan():
    ...
    flags=profiles[nmap_profile.get()]
    out=os.path.join(CAP_DIR,f"nmap_{tgt.replace('/','_')}_{int(time.time())}.log")
    log(scan_out,f"[*] nmap {' '.join(flags)} {tgt}")
    run_logged(["nmap",*flags,tgt], scan_out, out)

# ── attack helpers ──────────────────────────────────────────────────────────
def pick_target():
    sel=target_var.get()
    if not sel:
        messagebox.showinfo("Target","Pick a network"); return None
    return networks[int(sel)-1]

def handshake_monitor(cap,bssid):
    global attack_proc
    while attack_proc and attack_proc.poll() is None:
        try:
            o=subprocess.check_output(["aircrack-ng","-a","2","-w","/dev/null",
                                       "-b",bssid,cap],text=True,
                                       stderr=subprocess.DEVNULL,timeout=20)
            if "handshake" in o.lower():
...
def start_beacon():
    ...
    log(att_out,"[*] Beacon spam running – Stop to end")
...
Spinbox(wifi_box,from_=15,to=180,textvariable=scan_time,width=6).grid(row=1,column=1,sticky="w")
...
Label(nmap_box,text="nmap Target",bg=BGC,fg=NEON).grid(row=0,column=0,padx=4)
...
scan_out=scrolledtext.ScrolledText(ts,width=115,height=20,
...
Button(st,text="Refresh Stats",command=show_stats,
       bg=ACCENT,fg="white").pack(pady=4)
stats_out=scrolledtext.ScrolledText(st,width=115,height=12,
                                    bg="#0d0d17",fg=NEON,font=("Consolas",10))
stats_out.pack()
...
if __name__=="__main__":
    if os.geteuid()!=0:
        messagebox.showerror("Need root","Run with sudo.")
        sys.exit(1)
    root.mainloop()
