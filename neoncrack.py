#!/usr/bin/env python3
"""
NeonCrack v2.4-r4 – live logs, picker fix, Wifite-style WPS flag + Reset tab
...
root = Tk(); root.title("NeonCrack v2.4-r4"); root.configure(bg=BGC); root.geometry("1080x800")
...
def run_logged(cmd, widget, outfile=None):
    """launch cmd & stream its output into widget (+ optional tee)"""
...
def detect_wps(mon_iface, channel_list, dwell=3):
    """
    Wifite-style WPS sniff: iterate channels seen in the scan and run
    `wash -i <iface> -c <ch> --ignore-fcs -s -g --rx-timeout 1`.
    """
    hits=set()
    for ch in sorted(set(channel_list)):
        try:
            out=subprocess.check_output(
                ["timeout",str(dwell),"wash","-i",mon_iface,"-c",str(ch),
            ...
            continue
    log(scan_out, f"[*] WPS sniff pass complete – {len(hits)} APs flagged")
    return hits
...
def do_scan(channel_hop=False):
    ...
    base=parse_csv(csvp)
    wps_set=detect_wps(mon, [n[1] for n in base])
    ...
    for idx,(bssid,ch,essid,enc) in enumerate(base,1):
        flag="Y" if bssid in wps_set else "-"
    ...
    for idx,(_,_,essid,_,_) in enumerate(networks,1):
        target_menu["menu"].add_command(label=f"{idx} – {essid}",
...
def stop_attack():
    global attack_proc
    if attack_proc:
        os.killpg(os.getpgid(attack_proc.pid),signal.SIGTERM); attack_proc=None
...
def reset_toolkit(exit_after=False):
    """
    • Stop attack   • disable monitor-mode   • restart Wi-Fi services
    • Clear all text fields / vars          • optional exit
    """
    stop_attack()
    for svc in ("NetworkManager","wpa_supplicant"):
        subprocess.run(["systemctl","restart",svc],
                       stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL)
    for box in (scan_out, att_out, crack_out,
                hashid_out, cleaner_out, stats_out):
        box.delete("1.0", END)
    for v in (target_var, pcap_var, word_var,
              hash_input, iface_var):
        v.set("")
    refresh_iface_menu()
    log(scan_out,"[*] Toolkit reset – Wi-Fi services restarted.")
    if exit_after: root.quit()

# ── cracking & utilities (unchanged from r3) ──────────────────────────────────
...
style.configure("TNotebook.Tab",background="#1a1a1a",
                foreground=NEON,padding=8,font=FONT)

nb=ttk.Notebook(root); nb.pack(fill=BOTH,expand=True)
tabs={}
for key,label in [("scan","⚡ Scan"),("attack","⚔️ Attacks"),
                  ("crack","💥 Crack"),("hash","🔎 Hash ID"),
                  ("clean","🧹 Cleaner"),("stats","📊 Stats")]:
    fr=Frame(nb,bg=BGC); nb.add(fr,text=label); tabs[key]=fr
...
#  Reset tab
re=Frame(nb,bg=BGC); nb.add(re,text="♻️ Reset"); tabs["reset"]=re
Button(re,text="Reset NeonCrack",bg=ACCENT,fg="white",
       width=30,command=lambda:reset_toolkit(False)).pack(pady=12)
Button(re,text="Reset & Exit",bg="#ff0030",fg="white",
       width=30,command=lambda:reset_toolkit(True)).pack()
